# RED 消融
# variant3
# 在 variant2基础上，把图结点信息聚合方式设置成`w_1 * review_fea + fea`，多图信息聚合仍然采取拼接
...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.fc_user = nn.Linear(global_emb_size  * 5 * 1, global_emb_size * 5 * 1)
        self.fc_item = nn.Linear(global_emb_size  * 5 * 1, global_emb_size * 5 * 1)
        self.dropout = nn.Dropout(0.7)

        self.predictor = nn.Sequential(
            nn.Linear(global_emb_size * 5 * 1, global_emb_size * 5 * 1, bias=False),
            nn.ReLU(),
            nn.Linear(global_emb_size * 5 * 1, 5, bias=False),
        )
