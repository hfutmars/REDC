# RED 消融
# variant2
# 去除corr模块，结点信息传播不变，embedding不切割，各个子图实现embedding共享；
# 由于图学习方式的特殊性(拼接)，在最后图聚合的时候，将每个子图得到的表征切割[64,review_fea_size]，
# 这样得到5组[64,review_fea_size]的特征，之后将fea部分相加，再逐一跟每个review_fea拼接
# 这样做，保证GCN之后的特征维度跟RED一致；并且由于都是拼接，那么在预测的时候，和RED的拼接隔离的效果一致；
...
class Net(nn.Module):
    ...
    def forward(self, enc_graph_dict, users, items):
        ...
        for r in rate_list:
            # first layer
            feat_ = self.encoder_1[str(r)](enc_graph_dict[str(r)], weight_origin_all) 
            ...
            entangle_fea_temp,review_fea_temp = torch.split(feat_,[global_emb_size * 5 , global_review_size],dim=1)
            collect_entangle_fea.append(entangle_fea_temp)
            collect_review_fea.append(review_fea_temp)
        
        fea_sum_temp = collect_entangle_fea[0]
        for i in range(1,5):
            fea_sum_temp = fea_sum_temp + collect_entangle_fea[i]

        feat_all_new = []
        feat_all_new.append(fea_sum_temp)
        for i in range(0,5):
            feat_all_new.append(collect_review_fea[i])

        feat_new = torch.cat(feat_all_new,dim=-1)
