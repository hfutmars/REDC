...
class Data(object):
    ...
    def __init__(self, dataset_name, dataset_path, device, review_fea_size):
        ...
        self.train_enc_graph = self._generate_enc_graph_whole()
    ...
    def _generate_enc_graph_whole(self):
        user_item_r = np.zeros((self._num_user + self._num_item, self._num_item + self._num_user), dtype=np.float32)
        for i in range(len(self.train_rating_values)):
            user_item_r[[self.train_rating_pairs[0][i], self.train_rating_pairs[1][i]]] = self.train_rating_values[i]
        record_size = self.train_rating_pairs[0].shape[0]
        ...
        graph_whole = dgl.graph((rating_row, rating_col), num_nodes=self._num_user + self._num_item)
        ...
        c_temp = []
        c_temp.append(graph_whole.in_degrees())
        graph_whole.ndata['ci_r'] = _calc_norm(graph_whole.in_degrees(),0.5)
        
        c_sum_temp = _calc_norm(torch.sum(torch.stack(c,dim=0),dim=0),0.5)
        c_sum_mean_temp = _calc_norm(torch.sum(torch.stack(c,dim=0),dim=0),1)

        graph_whole.ndata['ci'] = c_sum_temp
        graph_whole.ndata['ci_mean'] = c_sum_mean_temp
...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.weight_origin = nn.Parameter(torch.Tensor(params.num_users + params.num_items, global_emb_size))

        self.encoder_1 = nn.ModuleDict(GCN_1())
        ...
        self.fc_user = nn.Linear(global_emb_size * 1, global_emb_size * 1)
        self.fc_item = nn.Linear(global_emb_size * 1, global_emb_size * 1)
        self.dropout = nn.Dropout(0.7)

        self.predictor = nn.Sequential(
            nn.Linear(global_emb_size * 1, global_emb_size * 1, bias=False),
            nn.ReLU(),
            nn.Linear(global_emb_size * 1, 5, bias=False),
        )
    ...
    def forward(self, enc_graph_dict, users, items):
        ...
        weight_origin_all = self.weight_origin

        feat_ = self.encoder_1(enc_graph_dict, weight_origin_all) 
        # second layer
