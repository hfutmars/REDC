...
class Data(object):
    ...
    def __init__(self, dataset_name, dataset_path, device, review_fea_size):
        ...
        # item_id = item_id + max_user_id，相当于将用户，物品视作是一类结点
        ...
        # train_datas = (user_id列表，item_id列表，评分列表)，元组
        ...
        # rating_pairs：[[uid,iid], [iid,uid]]; rating_values[raing, rating], 一个rating对应一对(user,item)
        ...
        ## find and collect extremely distributed samples， 寻找并收集
        ...
        # 字典, 记录物品-评论特征，{iid:{r: [reviews_fea] } }
    ...
    # 返回图数据，每个评分建立一个图，同时图的结点数据有两种，标准化和取平均
...
class Corr(nn.Module):
    ...
    def forward(self, feat_list):
        ...
        for r in [0, 1, 2, 3]:
            ...
            loss_gap_1_list.append(sim)
        loss_gap_1 = torch.stack(loss_gap_1_list, dim=0)
        loss_gap_1 = torch.mean(loss_gap_1, dim=0)
        ...
        for r in [0, 1, 2]:
            ...
            b = feat_list[r + 2]
            sim = torch.sum(a * b, dim=1)
            loss_gap_2_list.append(sim)
        loss_gap_2 = torch.stack(loss_gap_2_list, dim=0)
        loss_gap_2 = torch.mean(loss_gap_2, dim=0)
        ...
        for r in [0, 1]:
            ...
            b = feat_list[r + 3]
            sim = torch.sum(a * b, dim=1)
            loss_gap_3_list.append(sim)
        loss_gap_3 = torch.stack(loss_gap_3_list, dim=0)
        loss_gap_3 = torch.mean(loss_gap_3, dim=0)
        ...
        for r in [0]:
            ...
            b = feat_list[r + 4]
            sim = torch.sum(a * b, dim=1)
            loss_gap_4_list.append(sim)
        loss_gap_4 = torch.stack(loss_gap_4_list, dim=0)
        loss_gap_4 = torch.mean(loss_gap_4, dim=0)

        loss_12 = - self.activation(torch.sum((loss_gap_1 - loss_gap_2)))

        loss_23 = - self.activation(torch.sum((loss_gap_2 - loss_gap_3)))

        loss_34 = - self.activation(torch.sum((loss_gap_3 - loss_gap_4)))

        return loss_12 + loss_23 + loss_34
...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.fc_user = nn.Linear(global_review_size * 5 * 1,
                                 global_review_size * 5 * 1)
        self.fc_item = nn.Linear(global_review_size * 5 * 1,
                                 global_review_size * 5 * 1)
        self.dropout = nn.Dropout(0.7)
    ...
    def forward(self, enc_graph_dict, users, items):
        feat_all = []
        feat_all_user, feat_all_item = [], []
        rate_list = [1, 2, 3, 4, 5]
        weight_origin_all = torch.split(self.weight_origin,
                                        [global_emb_size, global_emb_size, global_emb_size, global_emb_size,
                                         global_emb_size], dim=-1)
        for r in rate_list:
            # first layer
            feat_ = self.encoder_1[str(r)](enc_graph_dict[str(r)], self.weight[str(r)])
            ...
            feat_all_user.append(feat_[users])
            feat_all_item.append(feat_[items])


        loss_user = self.corr_user(feat_all_user)
        loss_item = self.corr_item(feat_all_item)
        ...
        return pred_ratings, loss_user, loss_item
...
def evaluate(args, net, dataset, flag='valid', add=False, epoch=256):
    ...
    with torch.no_grad():
        ...
        if epoch <= g_epoch:
            ...
            pred_ratings, _, _ = net(dataset.train_enc_graph, u_list, i_list)
        else:
            pred_ratings, _, _ = net(dataset.train_enc_graph_updated, u_list, i_list)
...
def train(params):
    ...
    for iter_idx in range(1, 400):
        ...
        for idx in range(len(r_list)):
            ...
            if iter_idx <= g_epoch:
                pred_ratings, loss_corr_user, loss_corr_item = net(dataset.train_enc_graph, batch_user, batch_item)
            else:
                pred_ratings, loss_corr_user, loss_corr_item = net(dataset.train_enc_graph_updated, batch_user,
                                                                   batch_item)
            ...
            loss_f = loss + loss_corr_user + loss_corr_item
